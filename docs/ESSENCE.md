...
| **当前分支** | **主仓 = `<REPO>`（2026-09-16 起以它为主）**：独立 clone，唯一分支 `my2`=`846ac3c`（= 4614c0b 压平提交 + 主仓切换文档修订），独立构建指纹与归档成功件三方一致；旧仓 `root/ghostlock-app` = **冻结档案**（main 领先 origin/main 39 个未推送提交 + 32 项未提交改动，原地封存**勿用勿删**——历史文档引用它）；`my2` worktree 已注销退休（目录因句柄锁暂未物理删除，内有退休标记） |
