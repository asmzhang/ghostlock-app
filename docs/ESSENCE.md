...
| **当前分支** | **主仓 = `<REPO>`（2026-09-16 起以它为主）**：独立 clone，唯一分支 `my2`=`aa37ddf`（61 文件），独立构建指纹与 my2/归档成功件三方一致；旧仓 `root/ghostlock-app` = **冻结档案**（main 领先 origin/main 39 个未推送提交 + 32 项未提交改动，原地封存**勿用勿删**——历史文档引用它）；`<WORK_DIR>/my2` worktree 同内容留存 |
