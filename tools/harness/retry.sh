...
for r in $(seq 1 8); do
  ...
  tmo 220 adb -s $DEV shell "cd $D_TMP && GHOSTLOCK_W1_ATTEMPTS=3 ./ghostlock" > "$LOGDIR/gl_retry_r${r}.log" 2>&1
  echo "ADB_EXIT=$?" >> "$LOGDIR/gl_retry_r${r}.log"
  if grep -q "child is root!" "$LOGDIR/gl_retry_r${r}.log"; then
    echo "=== SUCCESS at round $r ==="
    sleep 8
    adb -s $DEV shell "grep -E \"kernelsu|kernelpatch\" /proc/modules; ls /data/adb/ap/ 2>/dev/null; cat $D_TMP/.ghostlock_ksu.log 2>/dev/null | tail -15"
    break
  fi
