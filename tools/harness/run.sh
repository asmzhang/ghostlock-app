#!/bin/bash
# GhostLock TUNE mode = exploration + test-optimization in one loop.
#   * success  -> lock params to gl_tuned.env (feeds USE mode), verify APatch
#                takeover, stop immediately
#   * panic    -> classify (fired-but-died), keep stats, next round
#   * safe-no-fire -> parameter ineffective, adjust and retry
# Known today: only GHOSTLOCK_SHIFT=-2 fires; firing usually panics; success
# is a timing/state event (~1/10). TUNE's job is to (a) hit a success, (b)
# persist the exact working params, (c) collect the diagnostics the root
# script already dumps (sig_enforce/dmesg) so the 5.10 path can be fixed.
# 设备与路径统一由 env.sh 提供（自动探测设备、以脚本目录为根）。
# 不再硬编码序列号与 <ARCHIVE_DIR> —— 这两处曾是 5/9 个脚本的重复来源。
...
# 且 app 界面建议 4,5，README 亦写 "defaults to the big cores (fallback 0/1)"。
# 故默认改为自动探测频率最高的**同簇**两个核心（走 adb 读设备，
# 本机的 /sys 不存在）；探测不到就退回 4/5，最后 0/1。
detect_core_pair() {
    [ -n "${DEV:-}" ] || { echo "4 5"; return; }
    local line
    line=$(timeout 20 adb -s "$DEV" shell \
    ...
    # marble 实测：4/5（2.49GHz 同簇）第 1 轮命中，而跨簇组合未被验证。
    # 用纯 shell 遍历（不依赖 gawk 的 asorti，busybox 环境也稳）。
    local pair="" f seq
...
stage() {
  ...
  for i in $(seq 1 8); do
    ...
    adb -s $DEV shell 'rm -f /data/local/tmp/kernelsu.ko /data/local/tmp/ksud' >/dev/null 2>&1
    ...
    adb -s $DEV shell 'mkdir -p /sdcard/ghostlock_ko' >/dev/null 2>&1
    adb -s $DEV push "$KO" /sdcard/ghostlock_ko/kernelpatch.ko >/dev/null 2>&1
    adb -s $DEV push "$KSUN_KO" /sdcard/ghostlock_ko/kernelsu.ko >/dev/null 2>&1
    adb -s $DEV push "$KSUD_BIN" /sdcard/ghostlock_ko/ksud >/dev/null 2>&1
...
for r in $(seq 1 $ROUNDS); do
  ...
  timeout 400 adb -s $DEV shell "cd /data/local/tmp && GHOSTLOCK_SHIFT=$SHIFT GHOSTLOCK_CORE=$CORE GHOSTLOCK_CONSUMER_CORE=$CCORE GHOSTLOCK_SKIP_POLICY_FIXUP=1 GHOSTLOCK_LAYOUT=A GHOSTLOCK_OWNER=${OWNER:-} GHOSTLOCK_SELF_W2=1 GHOSTLOCK_INSMOD_ONLY=1 GHOSTLOCK_HOLD=1 ./ghostlock" > "$LOGDIR/gl_tune_r${r}.log" 2>&1
  ...
  if grep -qE "child is root!|self is root" "$LOGDIR/gl_tune_r${r}.log"; then
    ...
    echo "--- klog (in-process loader diagnostics, /sdcard copy is shell-readable) ---"
    adb -s $DEV shell 'cat /sdcard/ghostlock_klog 2>/dev/null || cat /data/local/tmp/.ghostlock_klog 2>/dev/null || echo "(no klog)"'
    echo "--- su (kpatch sucompat should answer immediately) ---"
    adb -s $DEV shell 'su -c id 2>&1 | head -3'
    echo "--- root worker marker ---"
    adb -s $DEV shell 'ls -la /data/local/tmp/.ghostlock_root_alive 2>/dev/null || echo "(none)"'
    break
  elif [ -z "$u" ]; then
